...
(* More standard stuff *)
  ...
  | Tpat_record (l1,_),Tpat_record (l2,_) ->
      ...
      compats ps qs
  | Tpat_array (_, ps), Tpat_array (_, qs) ->
...
let simple_match d h =
  ...
  | Array (_, len1), Array (_, len2) -> len1 = len2
  | _, Any -> true
  | _, _ -> false
...
| {pat_desc = Tpat_array (k, omegas)} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_array (k, args)) q.pat_type q.pat_env::
    rest
      ...
      | Array (k, _) ->
          ...
          let rec try_arrays l =
            if List.mem l all_lengths then try_arrays (l+1)
              ...
              make_pat (Tpat_array (k, omegas l)) d.pat_type d.pat_env in
          try_arrays 0
      | _ -> Patterns.omega
...
let rec has_instance p = match p.pat_desc with
  ...
  | Tpat_construct (_,_,ps) | Tpat_tuple ps | Tpat_array (_, ps) ->
      has_instances ps
...
| Tpat_array (k, ps), Tpat_array (_, qs)
      when List.length ps = List.length qs ->
        let rs = lubs ps qs in
        make_pat (Tpat_array (k, rs)) p.pat_type p.pat_env
| _,_  ->
    raise Empty
...
module Conv = struct
  ...
  let conv typed =
    ...
    let rec loop pat =
      ...
      | Tpat_array (k, lst) ->
          mkpat (Ppat_array (k, List.map loop lst))
      | Tpat_lazy p ->
