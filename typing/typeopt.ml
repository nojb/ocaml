...
let array_kind k exp =
  match k with
  | Floatarray -> Pfloatarray
  | Genarray -> array_type_kind exp.exp_env exp.exp_type

let array_pattern_kind k pat =
  match k with
  | Floatarray -> Pfloatarray
  | Genarray -> array_type_kind pat.pat_env pat.pat_type
