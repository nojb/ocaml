    ...
    if (Is_white_hd (h)){
      ...
      if (t < No_scan_tag){
        ...
        *(Caml_state->gray_vals_cur)++ = v;
        if (Caml_state->gray_vals_cur >= Caml_state->gray_vals_end) realloc_gray_vals ();
      }else{
                                       ...
                                       int in_ephemeron, int *slice_pointers)
{
  ...
  if (Is_block (child) && Is_in_heap (child)) {
    ...
    if (Is_white_hd (chd)){
      ...
      *gray_vals_ptr++ = child;
      if (gray_vals_ptr >= Caml_state->gray_vals_end) {
...
static void mark_slice (intnat work)
{
  ...
  start = current_index;
  while (work > 0){
    if (v == 0 && gray_vals_ptr > Caml_state->gray_vals){
      CAMLassert (start == 0);
      v = *--gray_vals_ptr;
    ...
    if (v != 0){
      ...
      if (Caml_state->markhp == Caml_state->limit){
...
static void sweep_slice (intnat work)
{
  ...
  caml_gc_message (0x40, "Sweeping %"
                   ARCH_INTNAT_PRINTF_FORMAT "d words\n", work);
  while (work > 0){
    if (Caml_state->gc_sweep_hp < Caml_state->limit){
      hp = Caml_state->gc_sweep_hp;
      hd = Hd_hp (hp);
      work -= Whsize_hd (hd);
      Caml_state->gc_sweep_hp += Bhsize_hd (hd);
      switch (Color_hd (hd)){
      case Caml_white:
        ...
        Caml_state->gc_sweep_hp = (char *) caml_fl_merge_block (Val_hp (hp));
        break;
      case Caml_blue:
      ...
      CAMLassert (Caml_state->gc_sweep_hp <= Caml_state->limit);
    }else{
      Caml_state->chunk = Chunk_next (Caml_state->chunk);
      if (Caml_state->chunk == NULL){
        /* Sweeping is done. */
...
void caml_major_collection_slice (intnat howmuch)
{
  ...
  */
  ...
  if (Caml_state->dependent_size > 0){
    dp = (double) Caml_state->dependent_allocated * (100 + Caml_state->percent_free)
