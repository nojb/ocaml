...
let mk_g_opt f =
  mk_bool_flag "-g" false f "Record debugging information for exception backtrace"
...
let mk_labels f =
  mk_bool_flag "-labels" true f "Use commuting label mode"

let mk_linkall f =
  mk_bool_flag "-linkall" false f "Link all modules, even unused ones"

let mk_linscan f =
  mk_bool_flag "-linscan" false f "Use the linear scan register allocator"
...
let mk_modern f = [
  "-modern", Arg.Unit (fun () -> f true), " (deprecated) same as -labels"
]

let mk_alias_deps f =
  mk_bool_flag "-alias-deps" false f "Record dependencies for module aliases"

let mk_app_funct f =
  mk_bool_flag "-app-funct" true f "Enable applicative functors"

let mk_check_prims f =
  mk_bool_flag "-check-prims" true f "Check runtime for primitives"

let mk_float_const_prop f =
  mk_bool_flag "-float-const-prop" true f "Enable constant propagation for floating-point operations"
...
let mk_principal f =
  mk_bool_flag "-principal" false f "Check principality of type inference"

let mk_rectypes f =
  mk_bool_flag "-rectypes" false  f "Allow arbitrary recursive types"
...
let mk_strict_sequence f =
  mk_bool_flag "-strict-sequence" false f "Enforce that the Left-hand part of a sequence need not have type unit"
...
struct
  let list = [
    ...
    mk_app_funct F._app_funct;
    mk_float_const_prop F._float_const_prop;
    mk_noassert F._noassert;
