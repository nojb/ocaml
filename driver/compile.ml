...
let to_bytecode i Typedtree.{structure; coercion; _} =
  ...
  |> Profile.(record ~accumulate:true generate)
    (fun { Lambda.code = lambda; required_globals; toplevel_printers } ->
       lambda
       ...
       |> fun bytecode -> bytecode, required_globals, toplevel_printers
    )

let emit_bytecode i (bytecode, required_globals, toplevel_printers) =
  ...
  Misc.try_finally
    ...
    (fun () ->
       bytecode
       |> Profile.(record ~accumulate:true generate)
         (Emitcode.to_file oc i.module_name cmofile ~required_globals ~toplevel_printers);
    )
