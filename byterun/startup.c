...
int caml_attempt_open(_TCHAR **name, struct exec_trailer *trail,
                      int do_open_script)
{
  _TCHAR * truename;
  int fd;
...
static int parse_command_line(_TCHAR **argv)
{
  int i, j;
...
CAMLexport void caml_main(_TCHAR **argv)
{
  int fd, pos;
  ...
  value res;
  char * req_prims;
  _TCHAR * shared_lib_path, * shared_libs;
