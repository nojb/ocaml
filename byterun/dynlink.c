...
static _TCHAR * parse_ld_conf(void)
{
  _TCHAR * stdlib, * ldconfname, * wconfig, * p, * q;
  char * config;
#ifdef _WIN32
  struct _stati64 st;
#else
...
/* Open the given shared library and add it to shared_libs.
   Abort on error. */
static void open_shared_lib(_TCHAR * name)
{
  _TCHAR * realname;
  void * handle;
...
void caml_build_primitive_table(_TCHAR * lib_path,
                                _TCHAR * libs,
                                char * req_prims)
{
  _TCHAR * tofree1, * tofree2;
  _TCHAR * p;
  char * q;
