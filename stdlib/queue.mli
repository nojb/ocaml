...
(** [take_opt q] removes and returns the first element in queue [q]. Returns
    [None] if the queue is empty.

    @since 4.08 *)

val take : 'a t -> 'a
(** Same as {!take_opt}, but raises {!Empty} if the queue is empty. *)
...
(** [peek_opt q] returns the first element in queue [q], without removing
    it from the queue. Returns [None] if the queue is empty.

    @since 4.08 *)

val peek : 'a t -> 'a
(** Same as {!peek}, but raises {!Empty} if the queue is empty. *)
