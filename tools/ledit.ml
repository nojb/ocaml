...
module Cursor : sig
  ...
  let goto_first c = try while true do before c done with Failure -> ()

  let goto_last c = try while true do after c done with Failure -> ()
...
let parse_string s pos =
  ...
  if !i < String.length s && s.[!i] = '"' then
    Some (String.sub s pos (!i - pos), !i+1)
  else
    None

let skip_spaces s pos =
  let i = ref pos in
  while !i < String.length s && match s.[!i] with ' ' | '\t' -> true | _ -> false do
    incr i
  done;
  if !i < String.length s && s.[!i] = '#' then
    String.length s
...
let parse_command s pos =
  let i = ref pos in
  while !i < String.length s && match s.[!i] with 'a'..'z' | 'A'..'Z' | '-' -> true | _ -> false do
    incr i
  done;
  String.sub s pos (!i-pos), !i

type binding =
    B_string of string
  | B_comm of string

let parse_binding s pos =
  match
    if pos < String.length s && s.[pos] = '"' then
      match parse_string s (pos+1) with
      | Some (str, pos) -> Some (B_string str, pos)
      | None -> None
    else
      None
  with
  | Some _ as x -> x
  | None ->
      let c, pos = parse_command s pos in
      Some (B_comm c, pos)

let parse_line s =
  if 0 < String.length s && s.[0] = '"' then
    match parse_string s 1 with
    | Some (key, pos) ->
        let pos = skip_spaces s pos in
        if pos < String.length s && s.[pos] = ':' then
          let pos = skip_spaces s (pos+1) in
          match parse_binding s pos with
          | Some (binding, pos) ->
              let pos = skip_spaces s pos in
