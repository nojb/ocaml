...
let default_load_path =
  ref (Load_path.Inc.from_dirs
         [ Filename.current_dir_name; Config.standard_library ])
...
let add_path_for mdl dir =
  let old =
    try Hashtbl.find load_path_for mdl
    with Not_found -> Load_path.Inc.empty in
  Hashtbl.replace load_path_for mdl (Load_path.Inc.add_dir dir old)
