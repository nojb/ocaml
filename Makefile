...
parsing/parser_menhir.ml: boot/menhir/parser_menhir.ml parsing/parser_menhir.mlyp
	@if [ parsing/parser_menhir.mlyp -nt boot/menhir/parser_menhir.ml ]; then \
	  echo; \
	  tput setaf 3; tput bold; printf "Warning: "; tput sgr0; \
	  echo "Your 'parser_menhir.mlyp' file is more recent than the parser in 'boot/'."; \
	  echo "Its changes will be ignored unless you run 'make promote-menhir'."; \
	  echo; \
	fi
	cp $< parsing
